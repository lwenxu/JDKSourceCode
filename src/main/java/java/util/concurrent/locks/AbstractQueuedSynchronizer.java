...
public abstract class AbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements java.io.Serializable {
    ...
    static final class Node {
         ...
         *
         *   CONDITION:  该标识的结点处于等待队列中（不是同步队列），结点的线程等待在Condition上，当其他线程调用了Condition的signal()方法后，
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws == Node.SIGNAL)
            return true;
        //如果ws>0 则说明是任务取消 遍历前驱结点直到找到没有结束状态的结点
        if (ws > 0) {
            do {
                node.prev = pred = pred.prev;
            } while (pred.waitStatus > 0);
            ...
            //如果ws小于等于0又不是SIGNAL状态， 则将其设置为SIGNAL状态，代表该结点的线程正在等待唤醒。
            // 一般都是这种情况。因为节点被加入同步队列后的状态是 0
    ...
    private void doAcquireShared(int arg) {
        // 当前线程包装成一个共享锁节点放入同步队列  代码和互斥锁中的  acquiredQueued 方法基本一模一样
    ...
    */
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private Node addConditionWaiter() {
            Node t = lastWaiter;
            // 如果最后一个线程处于取消状态  删除取消的节点
            if (t != null && t.waitStatus != Node.CONDITION) {
                unlinkCancelledWaiters();
                t = lastWaiter;
            }
            // 把当前线程加入等待队列 并返回当前线程的节点
